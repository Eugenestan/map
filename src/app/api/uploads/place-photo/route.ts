...
/** Защита от заливки тысяч файлов с одного IP при добавлении места гостем. */
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit({
    key: `upload-place-photo:${getClientIp(request)}`,
    limit: RATE_LIMIT_PER_IP,
    windowMs: RATE_LIMIT_WINDOW_MS,
  });
  if (!rateLimit.allowed) {
    return createRateLimitResponse(
      "Слишком много загрузок фото с этого IP. Попробуйте позже.",
  ...
  try {
    ...
    console.error("uploads/place-photo: invalid multipart body", error);
  ...
  try {
    ...
    console.error("uploads/place-photo: cannot read file", error);
  ...
  try {
    ...
    console.error("uploads/place-photo: sharp failed", error);
    return NextResponse.json({ error: "Не удалось обработать изображение" }, { status: 400 });
  }

  const key = `places/${uuid()}.${extension}`;
  ...
  try {
    ...
    console.error("uploads/place-photo: S3 upload failed", error);
